            ...
            <Box className="table-container" style={{ marginBottom: 1.5 + 'rem' }}>
                <Table className="is-narrow">
                    ...
                    <Tfoot>
                        <Tr>
                            <Td>
                                <Button size="sm" mt={1} onClick={this.props.onAddLineButtonClick}><i className="fas fa-plus"></i></Button>
                            </Td>
                            <Td></Td>
                            <Td></Td>
                        </Tr>
        ...
        if (questname === defaultQuestName || questname.trim().length === 0) {
            return (
                <Box className="control">
                    <Input type="text" isInvalid value={questname} onChange={this.handleChange} />
                    <Text color="red" as="b">周回場所を入力してください。</Text>
                </Box>
            )
        }
        ...
        if (pos < 0) {
            return (
                <Box className="control">
                    ...
                    <Text className="help is-info">「剣の修練場 超級」「オケアノス 豊かな海」のようにスペースで区切る記述を推奨します。</Text>
                </Box>
            )
        }
        return (
...
class RunCountEditor extends React.Component {
    ...
    buildInputNode() {
        ...
        if (_runcount <= 0 || isNaN(_runcount)) {
            return (
                <Box>
                    <Input type="number" isInvalid min="0" value={this.props.runcount} onChange={this.handleChange} />
                    <Text color="red" as="b">周回数を設定してください。</Text>
                </Box>
            )
        }
        return (
            <Box className="control has-icons-right">
                <InputGroup>
                    <Input type="number" min="0" value={this.props.runcount} onChange={this.handleChange} />
                    <InputRightElement>
    ...
    render() {
        ...
        return (
            <Box>
                ...
                <Box mt={1}>
                    <Button size="sm" colorScheme="red" mr={1} onClick={this.handleClick}>-1000</Button>
                    <Button size="sm" colorScheme="red" mr={1} onClick={this.handleClick}>-100</Button>
                    <Button size="sm" colorScheme="red" mr={1} onClick={this.handleClick}>-10</Button>
                    <Button size="sm" colorScheme="red" mr={1} onClick={this.handleClick}>-1</Button>
                    <Button size="sm" colorScheme="blue" mr={1} onClick={this.handleClick}>+1</Button>
                    <Button size="sm" colorScheme="blue" mr={1} onClick={this.handleClick}>+10</Button>
                    <Button size="sm" colorScheme="blue" mr={1} onClick={this.handleClick}>+100</Button>
                    <Button size="sm" colorScheme="blue" onClick={this.handleClick}>+1000</Button>
                </Box>
            </Box>
        )
    }
...
class TweetButton extends React.Component {
    ...
    handleClick() {
        const tweetURL = `https://twitter.com/intent/tweet?text=${encodeURIComponent(this.props.reportText)}`
        window.open(tweetURL, "_blank")
    }

    render() {
        const tweetButton = <Button size="sm" colorScheme="twitter" onClick={this.handleClick}>ツイートする</Button>
        return (
            <Box mt={1}>{tweetButton}</Box>
        )
    }
}
...
export class EditBox extends React.Component {
    ...
    handleAddLineButtonClick() {
        ...
        const newline = {
            id: Math.max(...lines.map(line => { return line.id }), 0) + 1,
            order: Math.max(...lines.map(line => { return line.order }), 0) + 1,
            material: "",
            initial: 0,
            add: 0,
            reduce: 0,
...
`
    ...
    render() {
        return (
            <Box>
                <QuestNameEditor questname={this.state.questname} onQuestNameChange={this.handleQuestNameChange} />
                <RunCountEditor mb={1} runcount={this.state.runcount} onRunCountChange={this.handleRunCountChange} />
                <Box mt={1}>
                    {this.makeReportTable()}
                </Box>
                <ReportViewer {...this.state} />
                <TweetButton {...this.state} onShowTweetButton={this.handleShowTweetButton} />
            </Box>
        )
    }
}
